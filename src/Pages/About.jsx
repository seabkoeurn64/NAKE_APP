// src/Pages/About.jsx - PERFORMANCE OPTIMIZED
...
// ✅ Optimized ProfileImage Component
const ProfileImage = memo(({ prefersReducedMotion }) => {
  ...
  return (
    <div className="flex justify-center items-center p-4 lg:p-8">
      <div 
        className="relative group cursor-pointer performance-layer"
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        {/* Floating animation - conditionally rendered */}
        ...
        {/* Animated stars - conditionally rendered */}
        {!prefersReducedMotion && (
          <>
        ...
        <div className="relative">
              ...
              <div className="w-full h-full rounded-2xl bg-[#030014] relative overflow-hidden">
                ...
                {!imageLoaded && !imageError && (
                  <div className="absolute inset-0 bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center">
                    <div className="relative">
                      <UserCheck className="w-12 h-12 text-gray-600" style={!prefersReducedMotion ? { animation: 'ping 1s infinite' } : {}} />
          ...
          <div className="absolute -bottom-3 -right-3 bg-gradient-to-r from-[#6366f1] to-[#a855f7] rounded-xl p-3 shadow-2xl transform transition-all duration-300 group-hover:scale-110 group-hover:rotate-6" style={{
            ...
            <div className="flex items-center gap-2 text-white text-sm font-semibold">
              <div className="relative">
                <UserCheck className="w-4 h-4" style={!prefersReducedMotion ? { animation: 'pulse 2s infinite' } : {}} />
                {!prefersReducedMotion && (
          ...
          {/* Floating elements - conditionally rendered */}
          {!prefersReducedMotion && (
            <>
...
const StatCard = memo(({ icon: Icon, color, value, label, description, index, prefersReducedMotion }) => {
  const [isHovered, setIsHovered] = useState(false);
  ...
  useEffect(() => {
    if (localMounted && value > 0 && !prefersReducedMotion) {
  ...
  }, [value, index, localMounted, prefersReducedMotion]);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 bg-gray-900/80 backdrop-blur-2xl rounded-xl p-6 border border-white/10 overflow-hidden transition-all duration-500 hover:scale-105 h-full flex flex-col justify-between group-hover:border-white/20 performance-layer">
        {/* Animated background */}
...
const DownloadCVButton = memo(({ onClick, prefersReducedMotion }) => (
  <button 
    onClick={onClick}
    className="group relative px-5 py-3.5 rounded-xl bg-gradient-to-r from-[#6366f1] to-[#a855f7] text-white font-semibold text-base transition-all duration-500 hover:scale-105 active:scale-95 flex items-center justify-center gap-3 shadow-xl overflow-hidden hover:shadow-2xl w-full sm:w-auto sm:flex-1 performance-layer"
    style={{
      boxShadow: '0 0 20px rgba(168, 85, 247, 0.5)'
    }}
  >
...
const ViewWorkButton = memo(({ onClick, prefersReducedMotion }) => (
  <button 
    onClick={onClick}
    className="group relative px-5 py-3.5 rounded-xl border-2 border-[#a855f7] text-[#a855f7] font-semibold text-base transition-all duration-500 hover:scale-105 active:scale-95 flex items-center justify-center gap-3 hover:bg-[#a855f7]/10 overflow-hidden cursor-pointer hover:shadow-xl w-full sm:w-auto sm:flex-1 performance-layer"
  >
    {/* Hover background */}
...
const AboutBackground = memo(({ prefersReducedMotion }) => (
  <div className="absolute inset-0 overflow-hidden">
    <div className="absolute top-1/4 left-1/4 w-80 h-80 bg-[#6366f1] rounded-full blur-3xl opacity-10" 
         style={!prefersReducedMotion ? { animation: 'pulse 4s infinite' } : {}}></div>
    <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-[#a855f7] rounded-full blur-3xl opacity-5" 
         style={!prefersReducedMotion ? { animation: 'pulse 4s infinite 1s' } : {}}></div>
    <div className="absolute top-1/2 left-1/2 w-56 h-56 bg-[#8b5cf6] rounded-full blur-2xl opacity-5" 
         style={!prefersReducedMotion ? { animation: 'pulse 4s infinite 0.5s' } : {}}></div>
...
const AboutPage = () => {
  ...
  useEffect(() => {
    ...
    statsIntervalRef.current = setInterval(calculateStats, 2000);
  ...
  const renderedStatsCards = useMemo(() => 
    ...
    [statsData, prefersReducedMotion]
  );
        ...
        <div className="w-full max-w-6xl mx-auto">
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 xl:gap-16 items-center justify-between">
            ...
            <div className="w-full lg:w-1/2 space-y-6 lg:space-y-8 text-center lg:text-left order-2 lg:order-1">
              ...
              {/* Description section */}
              <div className="bg-gray-900/60 backdrop-blur-2xl rounded-xl p-6 border border-white/10 hover:border-white/20 transition-all duration-500 hover:shadow-xl group cursor-pointer performance-layer">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full">
                <DownloadCVButton onClick={handleDownloadCV} prefersReducedMotion={prefersReducedMotion} />
                <ViewWorkButton onClick={handleViewWork} prefersReducedMotion={prefersReducedMotion} />
              </div>
            </div>
